...
BOT_STATUS_NAME = os.getenv('BOT_STATUS_NAME', 'Braethe Cloud')
WATERMARK = os.getenv('WATERMARK', 'Braethe Cloud Team')
DISCORD_SUPPORT_LINK = os.getenv('DISCORD_SUPPORT_LINK', 'https://discord.gg/zC3xHcnj8r')
...
VPS_HOSTNAME = os.getenv('VPS_HOSTNAME', 'braethe-vps')  # Base hostname, append user ID
...
# VPS Plans: RAM tiers with auto-assigned disk and CPU resources
VPS_PLANS = {
    "2gb": {"ram": "2g", "cpu": "1", "disk": "10G"},
    "4gb": {"ram": "4g", "cpu": "2", "disk": "25G"},
    "8gb": {"ram": "8g", "cpu": "4", "disk": "50G"},
    "16gb": {"ram": "16g", "cpu": "8", "disk": "100G"},
}
...
def init_db():
    ...
    if 'expires_at' not in columns:
        cursor.execute("ALTER TABLE vps ADD COLUMN expires_at TEXT")
    if 'notified_level' not in columns:
        cursor.execute("ALTER TABLE vps ADD COLUMN notified_level INTEGER DEFAULT 0")
    cursor.execute('''
...
def add_vps(user_id, container_id, container_name, os_type, hostname, ssh_command, ram=DEFAULT_RAM, cpu=DEFAULT_CPU, disk=DEFAULT_DISK, expires_at=None):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        INSERT INTO vps (user_id, container_id, container_name, os_type, hostname, status, ssh_command, ram, cpu, disk, suspended, expires_at, notified_level)
        VALUES (?, ?, ?, ?, ?, 'running', ?, ?, ?, ?, 0, ?, 0)
...
def update_vps_expiry(container_id, expires_at):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('UPDATE vps SET expires_at = ? WHERE container_id = ?', (expires_at, container_id))
    conn.commit()
    conn.close()


def update_vps_notified(container_id, level):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute('UPDATE vps SET notified_level = ? WHERE container_id = ?', (level, container_id))
    conn.commit()
    conn.close()
...
def parse_expire(expire_str):
    """Parse expiry string like '4d', '1w', '1m', '30d' into a timedelta."""
    if not expire_str:
        return None
    match = re.match(r'(\d+)\s*([dwm])', expire_str.lower().strip())
    if match:
...
def format_time_left(expires_at_str):
    """Format the remaining time until expiration for display."""
...
async def capture_sshx_with_retry(container_id, max_retries=5):
    """Try to capture SSHX link with retries for reliability."""
    for attempt in range(max_retries):
        ...
        if exec_process:
            ...
            if ssh_line:
                return ssh_line
            # Kill the process if it didn't produce a link
            try:
                exec_process.kill()
...
async def verify_container_running(container_id):
    """Verify that a Docker container is actually running."""
    try:
        output = subprocess.check_output(
        ...
        logger.error(f"Container verification failed for {container_id}: {e}")
        return False


async def verify_sshx_installed(container_id):
    """Verify that SSHX is installed in the container."""
    try:
        proc = await asyncio.create_subprocess_exec(
            "docker", "exec", container_id, "which", "sshx",
            stdout=asyncio.subprocess.PIPE, stderr=asyncio.subprocess.DEVNULL
        )
        stdout, _ = await asyncio.wait_for(proc.communicate(), timeout=15.0)
        return proc.returncode == 0 and stdout.decode().strip() != ""
    except Exception as e:
        logger.warning(f"SSHX verification failed for {container_id}: {e}")
        return False
...
async def regen_ssh_command(interaction: discord.Interaction, vps_identifier, send_response=True, target_user=None):
    ...
    ssh_line = await capture_sshx_with_retry(container_id)
    if ssh_line:
        ...
        embed = discord.Embed(description="Failed to generate SSHX access link. Please try again or contact support.", color=discord.Color.red())
        embed.set_footer(text=WATERMARK, icon_url=bot.user.avatar.url if bot.user.avatar else None)
        view = View()
        view.add_item(Button(label="Discord Support", style=ButtonStyle.link, url=DISCORD_SUPPORT_LINK))
        if send_response:
...
async def manage_vps(interaction: discord.Interaction, vps_identifier, action, target_user=None):
    ...
    # Check expiration before allowing start
    if action == "start" and vps['expires_at'] and target_user == interaction.user:
        try:
            expires_at = datetime.fromisoformat(vps['expires_at'])
            if datetime.now(timezone.utc) > expires_at:
                embed = discord.Embed(
                    title="VPS Expired",
                    description="This VPS has expired and has been deleted. Contact Discord support to create a new VPS.",
    ...
    if action == "start":
        ...
        result = await async_docker_restart(container_id)
        if result:
            update_vps_status(container_id, "running")
            success = True
    if success:
...
async def reinstall_vps(interaction: discord.Interaction, vps_identifier, os_type, target_user=None):
    ...
    if new_container_id:
        ...
        if not await verify_container_running(new_container_id):
            embed = discord.Embed(description="Reinstall failed: Container did not start properly.", color=discord.Color.red())
            await interaction.followup.send(embed=embed, ephemeral=True)
            await async_docker_rm(new_container_id)
            return
        await async_install_sshx(new_container_id, os_type)
        await asyncio.sleep(15)  # Wait for install to complete
        if not await verify_sshx_installed(new_container_id):
            embed = discord.Embed(description="Reinstall failed: SSHX installation did not complete. Please try again or contact support.", color=discord.Color.red())
            embed.set_footer(text=WATERMARK, icon_url=bot.user.avatar.url if bot.user.avatar else None)
            view = View()
            view.add_item(Button(label="Discord Support", style=ButtonStyle.link, url=DISCORD_SUPPORT_LINK))
            await interaction.followup.send(embed=embed, view=view, ephemeral=True)
            await async_docker_stop(new_container_id)
            await asyncio.sleep(2)
            await async_docker_rm(new_container_id)
            logger.error(f"SSHX install verification failed for reinstall {new_container_id}")
            return
        ssh_line = await capture_sshx_with_retry(new_container_id)
        if ssh_line:
            add_vps(user_id, new_container_id, new_container_name, os_type, hostname, ssh_line, ram, cpu, disk, expires_at)
            os_name = get_os_details(os_type)[0]
...
# Create VPS helper
async def create_vps(interaction: discord.Interaction, os_type, plan_key="2gb", expires_at=None, target_user=None):
    if target_user is None:
        target_user = interaction.user
    ...
    plan = VPS_PLANS.get(plan_key, VPS_PLANS["2gb"])
    ram = plan["ram"]
    cpu = plan["cpu"]
    disk = plan["disk"]
    ...
    if not container_id:
        ...
        await interaction.followup.send(embed=embed, ephemeral=True)
        return
    await asyncio.sleep(8)  # Wait for container to fully start
    # Verify container is running
    if not await verify_container_running(container_id):
        embed = discord.Embed(description="Container failed to start properly. Please try again or contact an admin.", color=discord.Color.red())
        await interaction.followup.send(embed=embed, ephemeral=True)
        await async_docker_rm(container_id)
        return
    ...
    if not await verify_sshx_installed(container_id):
        embed = discord.Embed(description="SSHX installation failed. The VPS container was created but SSHX access could not be configured. Please contact support.", color=discord.Color.red())
        embed.set_footer(text=WATERMARK, icon_url=bot.user.avatar.url if bot.user.avatar else None)
        view = View()
        view.add_item(Button(label="Discord Support", style=ButtonStyle.link, url=DISCORD_SUPPORT_LINK))
        await interaction.followup.send(embed=embed, view=view, ephemeral=True)
        ...
        await asyncio.sleep(2)
        await async_docker_rm(container_id)
        # Also remove from DB if partially added
        delete_vps(container_id)
        logger.error(f"SSHX install verification failed for {container_id}")
        return
    ssh_line = await capture_sshx_with_retry(container_id)
    if ssh_line:
        add_vps(user_id, container_id, container_name, os_type, hostname, ssh_line, ram, cpu, disk, expires_at)
        os_name = get_os_details(os_type)[0]
        embed = discord.Embed(title="VPS Instance Created", description=f"OS: {os_name}\nResources: {ram} RAM | {cpu} CPU | {disk} Disk\n[Connect to VPS]({ssh_line})", color=discord.Color.green(), timestamp=datetime.now(timezone.utc))
        if expires_at:
            exp_display = format_time_left(expires_at)
            embed.add_field(name="Expires In", value=exp_display or "Unknown", inline=False)
        embed.set_footer(text=WATERMARK, icon_url=bot.user.avatar.url if bot.user.avatar else None)
        try:
            await target_user.send(embed=embed)
        except discord.Forbidden:
...
# Send expiration DM notification
async def send_expiration_dm(vps_row, warning_type):
    try:
        user = await bot.fetch_user(vps_row['user_id'])
        os_name = get_os_details(vps_row['os_type'])[0]
        if warning_type == "3d":
            title = "⚠️ VPS Expiration Warning (3 Days Left)"
            desc = f"Your VPS **{vps_row['container_name']}** ({os_name}) will expire in **3 days**. Please renew your service or contact Discord support."
            color = discord.Color.orange()
        elif warning_type == "2d":
            title = "⚠️ VPS Expiration Warning (2 Days Left)"
            desc = f"Your VPS **{vps_row['container_name']}** ({os_name}) will expire in **2 days**. Please renew your service or contact Discord support."
            color = discord.Color.orange()
        elif warning_type == "1d":
            title = "⚠️ VPS Expiration Warning (1 Day Left)"
            desc = f"Your VPS **{vps_row['container_name']}** ({os_name}) will expire in **1 day**. Please renew your service or contact Discord support."
            color = discord.Color.orange()
        elif warning_type == "expired":
            title = "Your VPS Has Expired"
            desc = f"Your VPS **{vps_row['container_name']}** ({os_name}) has expired and has been **deleted**. Contact Discord support to create a new VPS."
            color = discord.Color.red()
        else:
            return

        embed = discord.Embed(title=title, description=desc, color=color, timestamp=datetime.now(timezone.utc))
        embed.add_field(name="Resources", value=f"{vps_row['ram']} RAM | {vps_row['cpu']} CPU | {vps_row['disk']} Disk", inline=False)
        embed.set_footer(text=WATERMARK, icon_url=bot.user.avatar.url if bot.user.avatar else None)
        view = View()
        view.add_item(Button(label="Discord Support", style=ButtonStyle.link, url=DISCORD_SUPPORT_LINK))
        await user.send(embed=embed, view=view)
    except discord.Forbidden:
        logger.warning(f"Cannot DM user {vps_row['user_id']} for expiration notification (privacy settings)")
    except Exception as e:
        logger.error(f"Failed to send expiration DM to {vps_row['user_id']}: {e}")
...
async def admin_list(interaction: discord.Interaction):
    ...
    cursor = conn.cursor()
    cursor.execute('''
        SELECT u.username, v.container_id, v.container_name, v.os_type, v.hostname, v.status, v.ram, v.cpu, v.disk, v.suspended, v.expires_at, v.notified_level
    ...
    for row in all_vps[:25]:
        ...
        exp_text = ""
        if expires_at:
            exp_text = f"\nExpires: {format_time_left(expires_at) or 'Unknown'}"
        embed.add_field(
            name=f"{status_emoji} {username} - {container_name} ({os_type}) {suspended_text}",
            value=f"ID: ```{container_id}```\nHostname: {hostname}\nStatus: {status}\nResources: {ram} RAM | {cpu} CPU | {disk} Disk{exp_text}",
            inline=False
        )
    if len(all_vps) > 25:
...
@bot.tree.command(name="admin-users", description="Admin: List users with VPS counts")
@app_commands.guild_only()
...
async def admin_stats(interaction: discord.Interaction):
    ...
    cursor.execute('SELECT COUNT(*) FROM vps WHERE expires_at IS NOT NULL')
    num_expiring = cursor.fetchone()[0]
    ...
    embed.add_field(name="Expiring VPS", value=num_expiring, inline=True)
...
@bot.tree.command(name="admin-del-user", description="Admin: Delete all VPS for a user")
...
async def admin_delete_user(interaction: discord.Interaction, target_user: discord.User):
    ...
    # DM the user about their VPS deletion
    if deleted > 0:
        try:
            dm_embed = discord.Embed(
                title="Your VPS Has Been Deleted",
                description=f"An administrator has deleted {deleted} VPS instance(s) from your account.",
                color=discord.Color.red(),
                timestamp=datetime.now(timezone.utc)
            )
            dm_embed.set_footer(text=WATERMARK, icon_url=bot.user.avatar.url if bot.user.avatar else None)
            view = View()
            view.add_item(Button(label="Discord Support", style=ButtonStyle.link, url=DISCORD_SUPPORT_LINK))
            await target_user.send(embed=dm_embed, view=view)
        except discord.Forbidden:
            logger.warning(f"Cannot DM user {target_user.id} about deletion")
        except Exception as e:
            logger.error(f"Failed to DM user {target_user.id} about deletion: {e}")
...
async def admin_vps_info(interaction: discord.Interaction, target_user: discord.User, vps_identifier: str):
    ...
    container_id_display = f"```{container_id}```"
    embed.add_field(name="Container ID", value=container_id_display, inline=False)
    embed.add_field(name="Allocated Resources", value=f"{vps['ram']} RAM | {vps['cpu']} CPU | {vps['disk']} Disk", inline=False)
    if vps['expires_at']:
        time_left = format_time_left(vps['expires_at'])
        embed.add_field(name="Expires In", value=time_left or "Unknown", inline=True)
        embed.add_field(name="Expiration Notified", value=f"Level {vps['notified_level']}" if vps['notified_level'] else "No", inline=True)
    else:
        embed.add_field(name="Expires In", value="Never (24/7)", inline=True)
        embed.add_field(name="Expiration Notified", value="No", inline=True)
...
async def about(interaction: discord.Interaction):
    ...
    embed.add_field(
        ...
        value=(
            f"➜ **Name:** Braethe Cloud\n"
            "➜ **Version:** v2.0\n"
            "➜ **Developer:** INFINITE\n"
            "➜ **Framework:** Python • discord.py\n"
            "➜ **Uptime Status:** 🟢 Online & Stable\n"
            "➜ **VPS Plans:** 2GB | 4GB | 8GB | 16GB RAM\n"
            "➜ **Features:** VPS control, Docker management, real-time monitoring, expiration system, and more!"
        ),
        inline=False
    )
    ...
    # Developer Section
    embed.add_field(
        name="Developer • INFINITE",
        value=(
            "**INFINITE** is the creator of **INFINITE LABS**, focused on VPS, servers, Linux, "
            "Pterodactyl, Minecraft hosting, and server infrastructure.\n\n"
            "🔹 **Specialties:**\n"
            "   • VPS & Server Management / Infrastructure\n"
            "   • Docker & Containerization\n"
            "   • Hosting / Control Panel Systems\n"
            "   • QEMU Virtual Machines\n"
            "   • Discord Automation & Bot Development\n"
            "   • Minecraft Server Hosting & Optimization\n\n"
            "Focused on building, testing, and experimenting with reliable server and hosting setups."
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="INFINITE LABS",
        value=(
            "📺 **YouTube:** [INFINITE LABS](https://www.youtube.com/@infinite8labs)\n"
            "💻 **GitHub:** [INFINITE](https://github.com/nxtinfinite481-png)\n"
            "💬 **Discord Support:** [Join Braethe Cloud Support](https://discord.gg/zC3xHcnj8r)"
        ),
        inline=False
    )
    ...
    embed.add_field(
        name="Braethe Cloud",
        value=(
            "VPS, servers, Minecraft hosting, Linux, Pterodactyl, and server infrastructure. "
            "Build, test, and experiment with different hosting and server setups."
        ),
        inline=False
    )
    ...
    embed.set_footer(
        text=WATERMARK,
        icon_url="https://plain-apac-prod-public.komododecks.com/202609/06/0mootbbGU2YubiLIzvDU/image.png"
    )
    embed.set_thumbnail(
        url="https://plain-apac-prod-public.komododecks.com/202609/06/0mootbbGU2YubiLIzvDU/image.png"
    )
...
@app_commands.describe(vps_identifier="VPS ID or Name (defaults to first)", lines="Number of lines (default 50)")
...
# VPS Plan choices for slash commands
RAM_CHOICES = [
    app_commands.Choice(name="2GB RAM (10GB Disk)", value="2gb"),
    app_commands.Choice(name="4GB RAM (25GB Disk)", value="4gb"),
    app_commands.Choice(name="8GB RAM (50GB Disk)", value="8gb"),
    app_commands.Choice(name="16GB RAM (100GB Disk)", value="16gb"),
]


# Slash Commands
@bot.tree.command(name="create", description="Deploy a new VPS instance - default 2GB RAM plan, 24/7")
...
async def deploy(interaction: discord.Interaction, os_type: str):
    await create_vps(interaction, os_type, plan_key="2gb")


@bot.tree.command(name="admin-create", description="Admin: Create a VPS for a user with RAM tier and optional expiration")
@app_commands.describe(
    target_user="The target user",
    os_type="The OS type for the VPS",
    ram="Select RAM tier (disk auto-assigned)",
    expire="Expiration duration (e.g. 4d=4days, 1w=1week, 1m=1month). Leave empty for 24/7."
)
...
async def admin_create(interaction: discord.Interaction, target_user: discord.User, os_type: str, ram: str, expire: str = None):
    if not is_admin(interaction.user):
        embed = discord.Embed(description="This command is restricted to admins only.", color=discord.Color.red())
        await interaction.response.send_message(embed=embed, ephemeral=True)
        return
    ...
    expires_at = None
    if expire:
        delta = parse_expire(expire)
        if delta:
            expires_at = (datetime.now(timezone.utc) + delta).isoformat()
        else:
            embed = discord.Embed(
                description=f"Invalid expiration format: `{expire}`. Use formats like `4d` (4 days), `1w` (1 week), `1m` (1 month).",
                color=discord.Color.red()
            )
            await interaction.response.send_message(embed=embed, ephemeral=True)
            return
    ...
    await create_vps(interaction, os_type, plan_key=ram, expires_at=expires_at, target_user=target_user)
...
@bot.tree.command(name="ssh", description="Regenerate SSH session for your VPS")
@app_commands.describe(vps_identifier="VPS ID or Name (defaults to first)")
...
async def list_vps(interaction: discord.Interaction):
    ...
    for vps in vps_list[:25]:
        ...
        if vps['expires_at']:
            expires_text = f"\nExpires in: {format_time_left(vps['expires_at']) or 'Unknown'}"
        else:
            expires_text = "\nExpires in: Never (24/7)"
        embed.add_field(
            name=f"{status_emoji} {vps['container_name']} ({vps['os_type']}) {suspended_text}",
            value=f"ID: ```{vps['container_id']}```\nHostname: {vps['hostname']}\nStatus: {vps['status']}\nUptime: {uptime}\nResources: {vps['ram']} RAM | {vps['cpu']} CPU | {vps['disk']} Disk{expires_text}",
            inline=False
        )
    embed.set_footer(text=WATERMARK, icon_url=bot.user.avatar.url if bot.user.avatar else None)
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
@bot.tree.command(name="delete", description="Remove your VPS instance")
@app_commands.describe(vps_identifier="VPS ID or Name")
...
@bot.tree.command(name="admin-stop-all", description="Admin: Stop all running VPS instances")
@app_commands.guild_only()
...
async def help_cmd(interaction: discord.Interaction):
    embed = discord.Embed(title="Braethe Cloud - Help", color=discord.Color.blue(), timestamp=datetime.now(timezone.utc))
    embed.set_author(name=bot.user.name, icon_url=bot.user.avatar.url if bot.user.avatar else None)
    embed.set_footer(text=WATERMARK, icon_url=bot.user.avatar.url if bot.user.avatar else None)
    embed.add_field(name="**User Commands**", value="", inline=False)
    embed.add_field(name="/create <os>", value="Create a new 2GB VPS (10GB disk, 24/7)", inline=False)
    ...
    embed.add_field(name="/ssh [vps_id]", value="Regenerate SSH session", inline=False)
    embed.add_field(name="/reinstall <vps_id> [os]", value="Reinstall VPS with a new OS (keeps resources)", inline=False)
    embed.add_field(name="/delete <vps_id>", value="Remove a VPS", inline=False)
    ...
    embed.add_field(name="**VPS Plans**", value="2GB RAM (10GB Disk) | 4GB RAM (25GB Disk) | 8GB RAM (50GB Disk) | 16GB RAM (100GB Disk)", inline=False)
    if ADMIN_ID > 0:
        embed.add_field(name="**Admin Commands**", value="", inline=False)
        embed.add_field(name="/admin-create <user> <os> <ram> [expire]", value="Create a VPS for a user with RAM tier and optional expiration", inline=False)
        ...
        embed.add_field(name="/admin-users", value="List users with VPS counts", inline=False)
        ...
        embed.add_field(name="/admin-del-user <user>", value="Delete all VPS for a user", inline=False)
        ...
        embed.add_field(name="/admin-stop-all", value="Stop all running VPS instances", inline=False)
    embed.add_field(name="**Support**", value=f"[Join Discord Support]({DISCORD_SUPPORT_LINK})", inline=False)
    await interaction.response.send_message(embed=embed, ephemeral=True)
...
async def check_expirations():
    """Check all VPS for expiration and send DM notifications."""
    now = datetime.now(timezone.utc)
    vps_list = get_expiring_vps()
    for vps in vps_list:
        try:
            expires_at = datetime.fromisoformat(vps['expires_at'])
            time_left = expires_at - now
            container_id = vps['container_id']
            notified = vps['notified_level']

            if time_left.total_seconds() <= 0:
                # VPS has expired - stop, remove container, and delete from DB
                if vps['status'] != "deleted":
                    await async_docker_stop(container_id)
                    await asyncio.sleep(2)
                    await async_docker_rm(container_id)
                    delete_vps(container_id)
                    logger.info(f"VPS {container_id} expired - stopped, removed, and deleted from DB")
                if notified < 4:
                    await send_expiration_dm(vps, "expired")
                    update_vps_notified(container_id, 4)
                    logger.info(f"Sent expiration DM for {container_id}")
            elif time_left <= timedelta(days=3) and notified < 1:
                await send_expiration_dm(vps, "3d")
                update_vps_notified(container_id, 1)
                logger.info(f"Sent 3-day warning DM for {container_id}")
            elif time_left <= timedelta(days=2) and notified < 2:
                await send_expiration_dm(vps, "2d")
                update_vps_notified(container_id, 2)
                logger.info(f"Sent 2-day warning DM for {container_id}")
            elif time_left <= timedelta(days=1) and notified < 3:
                await send_expiration_dm(vps, "1d")
                update_vps_notified(container_id, 3)
                logger.info(f"Sent 1-day warning DM for {container_id}")
        except Exception as e:
            logger.error(f"Expiration check error for VPS {vps['container_id']}: {e}")
