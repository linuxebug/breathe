# Breathe Cloud VPS Deploy Bot

Breathe Cloud is a Discord VPS management bot by INFINITE LABS. It deploys and manages Docker-based VPS containers and provides SSHX web terminal access.
...
```bash
git clone https://github.com/linuxebug/breathe.git
cd breathe
```
...
BOT_STATUS_NAME=Breathe Cloud
WATERMARK=Breathe Cloud Team
DISCORD_SUPPORT_LINK=https://discord.gg/zC3xHcnj8r
DEFAULT_RAM=2g
DEFAULT_CPU=1
DEFAULT_DISK=10G
VPS_HOSTNAME=breathe-vps
```
...
```bash
nano /etc/systemd/system/breathe.service
```
...
```ini
[Unit]
Description=Breathe Cloud VPS Discord Bot
After=network.target docker.service

[Service]
User=root
WorkingDirectory=/root/breathe
ExecStart=/root/breathe/venv/bin/python /root/breathe/bot.py
Restart=always
RestartSec=5
...
systemctl enable breathe
systemctl restart breathe
```

Check the bot status:

```bash
systemctl status breathe --no-pager
```

View live logs:

```bash
journalctl -u breathe -f
```
...
| VPS Limit / User | 1 |
| Total VPS Limit | 50 |
| Hostname | breathe-vps |

## Supported Operating Systems

Breathe Cloud currently supports:
...
Breathe Cloud uses SSHX for web-based VPS terminal access.

When a VPS is created, Breathe Cloud automatically:
...
Breathe Cloud uses SQLite for persistent VPS and user data.
